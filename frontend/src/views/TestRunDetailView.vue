...
const exportReport = () => {
  if (!testRun.value || !queryResults.value) return
  
  const html = generateTestRunReport(testRun.value, queryResults.value)
  const filename = `test-run-${testRun.value.run_id}-${Date.now()}.html`
